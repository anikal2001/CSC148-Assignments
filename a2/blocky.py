...
def _block_to_squares(board: Block) -> List[Tuple[Tuple[int, int, int],
    ...
    new_lst = []
    if board.children == []:
        new_tup = (board.colour, board.position, board.size)
        new_lst.append(new_tup)
    else:
        for child in board.children:
            new_lst.extend(_block_to_squares(child))
    return new_lst
...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Calls the process_event method for the given current_player, for the
        desired event.
        """
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """Prompts the user to make a move, then returns an updated version
        of the current GameState based on in-game events.

        If no move was made, return the unchanged GameState.
        """
...
class AnimateMoveState(GameState):
    ...
    def update(self) -> GameState:
        """Returns the next GameState based on whether an animation is currently
        running or not.
        """
    ...
    def render(self, renderer: Renderer) -> None:
        """Renderer for the moves of the game. Draws on the board an outline
        around the desired block of the current move, displays the image
        representing the desired move, and updates the status message respective
        to the move.
        """
...
class GameOverState(GameState):
    ...
    def update(self) -> GameState:
        """Returns self, does not alter in any way
        """
        # Nothing to change
        return self

    def render(self, renderer: Renderer) -> None:
        """Renders the final score and the winner of the game after the game
        has ended.
        """
        x = 10
        y = 10
