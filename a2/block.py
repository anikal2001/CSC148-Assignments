...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        if self.children != []:
            c_size = self._child_size()
            self.children[1]._update_children_positions(position)
            self.children[0]._update_children_positions((position[0] + \
                                                         c_size, position[1]))
            self.children[2]._update_children_positions((position[0],
                                                         position[1] + c_size))
            self.children[3]._update_children_positions((position[0] + c_size,
                                                         position[1] + c_size))
    ...
    def smash(self) -> bool:
        ...
        if not self.smashable():
            ...
            self.colour = None
            for i in range(0, 4):
                new_block = Block(children_positions[i], child_size,
                                  COLOUR_LIST[random.randint(0, 3)],
                                  self.level + 1, self.max_depth)
                self.children.append(new_block)
            for i in range(0, 4):
                rand_num = random.random()
                if rand_num < math.exp((-0.25 * self.children[i].level)):
    ...
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        if self.level == self.max_depth and self.children == []:
    ...
    def combine(self) -> bool:
        ...
        color_dict = {}  # maps colors to int
        for child in self.children:
            if child.colour not in color_dict:
                color_dict[child.colour] = 1
            else:
                color_dict[child.colour] += 1
        num_lst = list(color_dict.values())
        for color in color_dict:
            if color_dict[color] > 1 and num_lst.count(color_dict[color]) == 1:
                self.colour = color
                self.children = []
                return True
        return False
