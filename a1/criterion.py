    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        for i in range(len(answers)):
            for j in range(i + 1, len(answers)):
                if answers[i].is_valid(question) and answers[j].is_valid(
                        question):
...
class HeterogeneousCriterion(HomogeneousCriterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        avg = HomogeneousCriterion.score_answers(self, question, answers)
        return 1.0 - avg
...
class LonelyMemberCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        for answer in answer_set:
            count = 0
            for ans_l in answers:
                if ans_l.content == answer.content:
                    count += 1
            if count == 1:
                return 0.0
